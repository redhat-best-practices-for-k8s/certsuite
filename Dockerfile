FROM --platform=$BUILDPLATFORM registry.access.redhat.com/ubi9/ubi:9.5@sha256:3ef6a09c90ba5abb6011d0e5d2a797dd44f4c44143a6713479af8d7ac33140e1 AS build
...
ENV \
	...
	GOPATH=/root/go
ENV GO_BIN_URL_x86_64=${GO_DL_URL}/go1.24.2.linux-amd64.tar.gz
ENV GO_BIN_URL_aarch64=${GO_DL_URL}/go1.24.2.linux-arm64.tar.gz
...
RUN \
	if [ "$(uname -m)" = x86_64 ]; then \
		...
		&& rm -rf /usr/local/go \
		&& tar -C /usr/local -xzf ${TEMP_DIR}/go1.24.2.linux-amd64.tar.gz; \
	elif [ "$(uname -m)" = aarch64 ]; then \
		wget --directory-prefix=${TEMP_DIR} ${GO_BIN_URL_aarch64} --quiet \
		&& rm -rf /usr/local/go \
		&& tar -C /usr/local -xzf ${TEMP_DIR}/go1.24.2.linux-arm64.tar.gz; \
	else \
