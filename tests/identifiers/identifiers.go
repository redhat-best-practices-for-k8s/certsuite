...
that Node's kernel may not have the same hacks.'`,
	...
	TestSingleNamespacedOperatorInstallationInTenantNamespace = AddCatalogEntry(
		...
		common.OperatorTestKey,
		`Verifies that only single-namespace operators are installed in a tenant-dedicated namespace. The test fails if this namespace contains any cluster-wide operator, operarnds of any other operator not installed in this namespace, or pods unrelated to any operator.`,
