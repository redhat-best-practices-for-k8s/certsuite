...
that Node's kernel may not have the same hacks.'`,
	...
	TestSingleNamespacedOperatorInstallationInTenantNamespace = AddCatalogEntry(
		...
		common.OperatorTestKey,
		`Verifies that only single-namespace operators are installed in a tenant-dedicated namespace. The test fails if this namespace contains all-namespaced operators, cluster-wide operators, or pods unrelated to any operator.`,
