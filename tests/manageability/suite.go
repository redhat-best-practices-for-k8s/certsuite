...
func LoadChecks() {
	...
	checksadapter.AddCheck(checksGroup, "manageability-containers-image-tag", checksfn.CheckImageTag, &env,
		skipIfNoContainersFn)

	checksadapter.AddCheck(checksGroup, "manageability-container-port-name-format", checksfn.CheckPortNameFormat, &env,
		skipIfNoContainersFn)
}
