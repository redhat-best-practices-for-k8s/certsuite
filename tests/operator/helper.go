...
func getAllOperatorsBy(namespace string) (operatorsInNamespace []*provider.Operator) {
	for _, operator := range env.AllOperators {
		if operator.Namespace == namespace {
...
func checkValidOperatorInstallation(namespace string) (isDedicatedOperatorNamespace bool, validInstalledOperators, invalidInstalledOperators, invalidNonInstalledOperators, unlabeledInstalledOperators, podsNotBelongingToOperators []string) {

	for _, namespace := range env.Namespaces {
		// 1. operator installation checks
		for _, operator := range getAllOperatorsBy(namespace) {
			if namespace == operator.Csv.Annotations["olm.operatorNamespace"] {
				if checkIfOperatorUnderTest(operator) {
					if isSingleNamespacedOperator(operator) || isMultiNamespacedOperator(operator) {
						validInstalledOperators = append(validInstalledOperators, operator.Name)
					} else {
						invalidInstalledOperators = append(invalidInstalledOperators, operator.Name)
					}
				} else {
					unlabeledInstalledOperators = append(unlabeledInstalledOperators, operator.Name)
				}
			} else {
				invalidNonInstalledOperators = append(invalidNonInstalledOperators, operator.Name)
			}
		}
		// 2. existing pods check
		podsBelongingToNoOperators, err := findPodsNotBelongingToOperators(namespace)
		if err != nil {
			return false, validInstalledOperators, invalidInstalledOperators, invalidNonInstalledOperators, unlabeledInstalledOperators, podsNotBelongingToOperators
		}

		return len(podsBelongingToNoOperators) == 0 && len(validInstalledOperators) != 0, validInstalledOperators, invalidInstalledOperators, invalidNonInstalledOperators, unlabeledInstalledOperators, podsNotBelongingToOperators
	}

	return isDedicatedOperatorNamespace, validInstalledOperators, invalidInstalledOperators, invalidNonInstalledOperators, unlabeledInstalledOperators, podsNotBelongingToOperators
}
