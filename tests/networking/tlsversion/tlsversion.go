...
func ProbeServicePortTLS(address string, port int32, policy TLSPolicy) TLSProbeResult {
	...
	// Step 1: Try connecting at the policy's minimum version
	connMin, errMin := dialTLS(endpoint, policy.MinTLSVersion, tls.VersionTLS13)
	if errMin != nil {
		return classifyMinVersionError(errMin, policy)
	}
	connMin.Close()
...
// non-TLS responses (plain HTTP), and genuine TLS handshake failures (server doesn't
// support the required minimum version).
...
func ProbeServicePortViaExec(ch clientsholder.Command, ctx clientsholder.Context, address string, port int32, policy TLSPolicy) TLSProbeResult {
	...
	if errMin != nil {
		return TLSProbeResult{
			...
			IsTLS:     false,
			Reachable: false,
			Reason:    fmt.Sprintf("exec probe failed: %v", errMin),
		}
	}
...
// did not negotiate the policy's minimum version. Checks for connection refused,
// partial TLS handshake output, or non-TLS responses to determine the appropriate
// compliance result.
func classifyExecNoMinVersion(stdout string, policy TLSPolicy) TLSProbeResult {
	...
	if strings.Contains(stdout, "SSL handshake") || strings.Contains(stdout, "Protocol") {
		return TLSProbeResult{
			Compliant:     false,
			IsTLS:         true,
			Reachable:     true,
...
func CheckServiceTLSCompliance(check *checksdb.Check, env *provider.TestEnvironment, policy TLSPolicy) (
	...
	for _, svc := range env.Services {
		...
		for _, port := range svc.Spec.Ports {
			...
			// If Go TLS probe couldn't connect, try exec fallback via probe pod
			if !result.Reachable {
