...
func checkPodPortTLS(check *checksdb.Check, put *provider.Pod, ch clientsholder.Command,
	...
	var listeningPorts []netutil.PortInfo
	for _, container := range put.Containers {
		ports, err := netutil.GetListeningPorts(container)
		if err != nil {
			check.LogError("Failed to get container %q listening ports, err: %v", container, err)
	...
	for _, listeningPort := range listeningPorts {
