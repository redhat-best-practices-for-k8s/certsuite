...
func ProcessContainerIpsPerNet(containerID *provider.Container,
	...
	if len(ipAddressesFiltered) == 0 {
		...
		logrus.Debugf("Skipping %s, Network %s because no multus IPs are present", containerID.String(), netKey)
		return
	}
	...
	if entry.TesterSource.ContainerIdentifier == nil {
		logrus.Debugf("%s selected to initiate ping tests", containerID.String())
	...
	for netName, netUnderTest := range netsUnderTest {
		...
		for _, aDestIP := range netUnderTest.DestTargets {
			logrus.Debugf("%s ping test on network %s from ( %s  srcip: %s ) to ( %s dstip: %s )",
				aIPVersion, netName,
				netUnderTest.TesterSource.ContainerIdentifier.String(), netUnderTest.TesterSource.IP,
				aDestIP.ContainerIdentifier.String(), aDestIP.IP)
			...
			claimsLog = claimsLog.AddLogLine("%s ping test on network %s from ( %s  srcip: %s ) to ( %s dstip: %s ) result: %s",
				aIPVersion, netName,
				netUnderTest.TesterSource.ContainerIdentifier.String(), netUnderTest.TesterSource.IP,
				aDestIP.ContainerIdentifier.String(), aDestIP.IP, result.String())
			if err != nil {
