...
// ResetCache clears the cached resources. It does NOT clear the certification
// validator, as that depends on static metadata (offline DB) rather than
// cluster state.
func ResetCache() {
	cacheMutex.Lock()
...
func buildCRInstances(resources *checks.DiscoveredResources) map[string]map[string][]string {
	...
	for i := range resources.CRDs {
		...
		go func(crd *apiextv1.CustomResourceDefinition) {
			defer wg.Done()
			nsCRs := getCRInstances(crd, clients)
			if len(nsCRs) > 0 {
...
func getCRInstances(crd *apiextv1.CustomResourceDefinition, clients *clientsholder.ClientsHolder) map[string][]string {
